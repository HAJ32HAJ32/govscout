...
1. Create an immutable release under `/opt/govscout/releases/<version>` and point `/opt/govscout/current` to it.
...
3. Install `govscout.service` as `/etc/systemd/system/govscout.service` and the Caddy snippet as `/etc/caddy/Caddyfile` (or import it from the main Caddyfile).
...
Before every upgrade of an existing installation, stop the service and make a
private, timestamped database backup. On a first installation, verify that the
database path does not exist and skip the `cp`; startup creates and migrates it.
...
```console
sudo systemd-analyze verify /etc/systemd/system/govscout.service
