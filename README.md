...
The FCA-first discovery, enrichment, QC and review pipeline is available, while production drafting remains deliberately **fail-closed**. `/today` presents only research and human review work; dormant draft endpoints and commands still return `LINT_NOT_READY` before capacity is reserved or Gmail is contacted.
...
- Operator-researched official websites are append-only evidence and never overwrite the FCA-sourced website; reprocessing is a separate explicit, bounded action tied to current FCA, Collector-import and Companies House evidence.
...
When the FCA record has no usable website, `/today` lets the operator record a canonical official HTTPS URL together with its supporting source URL, justification and actor. That assertion is immutable, bound to the current FCA source hash and accepted Collector import, and displayed separately from `fca_firms.website_url`; it never rewrites FCA provenance. Reprocessing must then be requested as a second explicit action. The separate queue binds the original processing job, exact website assertion, latest current Companies House verification attempt and canonical input hash. Changed, superseded, withdrawn, archived or stale dependencies fail before website access and are checked again before enrichment or QC is persisted. Withdrawal appends a new event rather than mutating or deleting the assertion.
