...
Each enrichment run computes a 0-100 score and a HOT (>=75) / WARM (>=55) / COOL (below) label, shown on `/today` as Hot/Warm/Cool alongside the number. The score is the bounded sum of present evidence weights: FCA regulation (+40), visible AI mentions (+30), a privacy page silent on AI use (+15), incorporation age of at least three years (+10), and recognised analytics or chat tooling on the homepage (+5). It remains a first working signal rather than a finished model; thresholds should be revisited only after observing real target-market results. `/today` shows each signal's weight next to its plain-English verdict so the total can be audited line by line — see [`docs/today-redesign.md`](docs/today-redesign.md) for the full page design.
...
`/today` also supports manual, evidence-gated contact capture. An operator may assert a contact name, email and/or phone only with a canonical source URL and justification; at least one contact field is required. Assertions and withdrawals append immutable history bound to the current FCA identity and accepted Collector import. GovScout does not guess contacts, and this research evidence does not by itself create a sendable lead. Collector-side pre-fill of the FCA API's phone number and contact name remains deferred because it requires a separately built and redistributed desktop Collector release.
